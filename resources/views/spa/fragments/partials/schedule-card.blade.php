...
<section class="box-schedule {{ $s->is_active ? '' : 'inactive' }}">
    ...
    <div class="cover-schedule">
        <div class="desc-schedule">
            <h4>{{ $s->title }}</h4>
            <h5>{{ $typeLabels[$s->schedule_type] ?? '-' }} · {{ $s->getScheduleLabel() }}</h5>
        </div>
        <div class="sched-actions">
            <button class="btn-toggle-schedule" data-id="{{ $s->id }}"
    ...
    <div class="thumb-schedule">
        <div class="thumb-cover-schedule"></div>
