...
<div class="neo-dashboard rtd-dashboard">
    <div class="qz-container">
        ...
        <div
            ...
            <div class="qz-breadcrumb" style="margin-bottom: 0;">
                <a href="?page=materi&main_id={{ $subMateri->materi->mainMateri->id ?? '' }}"
                    class="link-spa">{{ $subMateri->materi->mainMateri->title ?? '-' }}</a>
                <span>/</span>
                <a href="?page=submateri&materi_id={{ $subMateri->materi_id }}"
                    class="link-spa">{{ $subMateri->materi->title ?? '-' }}</a>
                <span>/</span>
            ...
            <div class="qz-result-screen" id="qz-result-screen" style="display:none;">
                <div class="qz-result-circle" id="qz-result-circle">
                    <svg viewBox="0 0 120 120">
                        <circle class="qz-rc-bg" cx="60" cy="60" r="52" />
                        <circle class="qz-rc-fill" id="qz-rc-fill" cx="60" cy="60" r="52" />
                    </svg>
                ...
                <div class="qz-result-actions">
                    <button class="qz-btn qz-btn-retry" id="qz-retry" style="display:none;"
                        onclick="loadPage('quiz',{submateri_id:{{ $subMateri->id }}})">
                    ...
                    @else
                        <a href="?page=submateri&materi_id={{ $subMateri->materi_id }}"
                            class="link-spa qz-btn qz-btn-next-materi">
...
<script>
    (function () {
        ...
        const timerDisplay = document.getElementById('qz-timer-display');
        const timeVal = document.getElementById('qz-time-val');
        ...
        function startTimer() {
            ...
            timerInterval = setInterval(() => {
                ...
                if (timeVal) timeVal.textContent = formatTime(elapsedSeconds);
            }, 1000);
        }
        ...
        if (!form.classList.contains('qz-hidden')) {
            startTimer();
        }
        ...
        if (retakeBtn) {
            retakeBtn.onclick = () => {
                ...
                elapsedSeconds = 0;
                if (timeVal) timeVal.textContent = "00:00";
                startTimer();
                show(0);
            };
        }
        ...
        form.onsubmit = async (e) => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    // Show result
                    stopTimer();
                    const timerDisp = document.getElementById('qz-timer-display');
                    ...
                    const timeResultP = document.getElementById('qz-result-time');
                    if (timeResultP) {
                        timeResultP.style.display = 'block';
                        document.getElementById('qz-final-time').textContent = formatTime(elapsedSeconds);
                    }
